...
const mapStarships = (input) => {
  ...
  var result = input.map(getMapStarships);
  function getMapStarships(item) {
    if (item.cost_in_credits === "unknown") {
      ...
      return (
        ...
        item.manufacturer +
        " - cost: " +
        item.cost_in_credits.replace(/\B(?=(\d{3})+(?!\d))/g, ",") +
        " credits"
      );
    }
  }
...
const filterStarships = (input) => {
  ...
  var result = input.filter(getFilterStarships);
  function getFilterStarships(item) {
    return item.passengers < 10 && item.crew > 1;
  }
  return result;
};
...
const reduceStarships = (input) => {
  ...
  var costArray = input.map(getCost);
  function getCost(item) {
    if (item.cost_in_credits === "unknown") {
      return 0;
    } else {
      return Number(item.cost_in_credits);
    }
  }
  var totalCost = costArray.reduce(getReduceStarships);
  function getReduceStarships(total, num) {
    return total + num;
  }
  var result = totalCost.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
  return "The cost of all starships is " + result + " credits";
};
